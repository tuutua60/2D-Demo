...
public class BinaryDataMgr
{
    ...
    {
        LoadTable<T_AI_InfoContainer, T_AI_Info>();
        LoadTable<T_DamageInfoContainer, T_DamageInfo>();
        LoadTable<T_PlayerLevelInfoContainer, T_PlayerLevelInfo>();
        LoadTable<T_EnemyInfoContainer, T_EnemyInfo>();
        
        LoadTable<T_Level_Total_InfoContainer, T_Level_Total_Info>();
        LoadTable<T_Level_Section_InfoContainer, T_Level_Section_Info>();
        LoadTable<T_Item_InfoContainer, T_Item_Info>();
    }
    ...
    public void LoadTable<T,K>()
    {
        ...
        using (FileStream fs = File.Open(DATA_BINARY_PATH + typeof(K).Name + ".tuu", FileMode.Open, FileAccess.Read))
        {
    ...
    public T LoadData<T>(string fileName) where T:class
    {
        ...
        if( !File.Exists(SAVE_PATH + fileName + ".tuu") )
            return default(T);

        T obj;
        using (FileStream fs = File.Open(SAVE_PATH + fileName + ".tuu", FileMode.Open, FileAccess.Read))
        {
            BinaryFormatter bf = new BinaryFormatter();
